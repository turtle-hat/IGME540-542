...
{
	// Sample the albedo and alpha map texture at this pixel
    float4 sampleAA = MapAlbedoAlpha.Sample(BasicSampler, input.uv * uvScale + uvPosition);
    float finalAlpha = sampleAA.a;
	
	// Discard if alpha is below the threshold
    ...
    float3 sampleAlbedo = pow(sampleAA.rgb, 2.2f); // Gamma uncorrected so it gets the expected value after end correction
