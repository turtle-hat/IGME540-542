...
void Game::CreateFoliage()
{
	...
	fParams.segmentLengthVariance		= 1.0f;
	fParams.segmentLengthMultiplier		= 0.9f;
	fParams.segmentWidth				= 1.0f;
	fParams.segmentWidthVariance		= 0.25f;
	fParams.segmentWidthMultiplier		= 0.65f;
	...
	fParams.leafDistance				= 3.0f;
	fParams.leafDistanceVariance		= 0.5f;
	fParams.leafDistanceMultiplier		= 0.4f;
	fParams.leafWidth					= 5.0f;
	fParams.leafWidthVariance			= 1.0f;
	fParams.leafWidthMultiplier			= 0.6f;
	fParams.leafAngleChange				= GOLDEN_ANGLE;
	fParams.leafAngleChangeVariance		= 1.0f;
		...
		for (int i = 0; i < foliages.size(); i++) {
			...
			if (ImGui::TreeNode("", "(%06d) %s", i, foliages[i]->GetName())) {
				...
				if (ImGui::TreeNode("Parameters")) {
					...
					ImGui::SetItemTooltip("Average distance between leaf quads on a branch");

					if (ImGui::SliderFloat("Leaf Distance Variance", &params.leafDistanceVariance, 0.0f, params.leafDistance, "%.2f")) {
						paramsDirty = true;
					}
					ImGui::SetItemTooltip("Width of random range for leaf distance");

					if (ImGui::DragFloat("Leaf Distance Multiplier", &params.leafDistanceMultiplier, 0.01f, 0.0f, 3.0f, "%.2f", ImGuiSliderFlags_Logarithmic)) {
						paramsDirty = true;
					}
