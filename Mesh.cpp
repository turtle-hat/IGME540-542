...
void Mesh::InitializeBuffers(Vertex* _vertices, unsigned int _vertexCount, unsigned int* _indices, unsigned int _indexCount)
{
	...
	vertexBuffer = Graphics::CreateStaticBuffer(sizeof(Vertex), _vertexCount, _vertices);
	indexBuffer = Graphics::CreateStaticBuffer(sizeof(unsigned int), _indexCount, _indices);
	...
	ibView.SizeInBytes = sizeof(unsigned int) * _indexCount;
